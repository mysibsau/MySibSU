...
export default function LibrarySearch(props){
    ...
    return(
        <View style={{flex: 1, backgroundColor: theme.primaryBackground}}>
            ...
            <>
            {books.physical.length === 0 ? <Text style={{ fontSize: 18, alignSelf: 'center', marginTop: 20, color: theme.labelColor}}>{locale['empty']}</Text> : null}
            <FlatList 
                data={books.physical}
                renderItem={({ item }) => 
                    <View style={{width: w * 0.9, backgroundColor: theme.blockColor, borderRadius: 15, elevation: 5, marginTop: 10, alignSelf: 'center', padding: 10}}>
                        <Text style={{color: theme.labelColor,  fontSize: 16}}>{item.name}</Text>
                        <Text style={{color: 'gray',  fontSize: 16}}>{item.author}</Text>
