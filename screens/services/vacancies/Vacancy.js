    ...
    return(
        <View style={{flex: 1}}>
            ...
            <View style={{flex: 1, backgroundColor: theme.primaryBackground}}>
                ...
                <Text style={{  fontSize: 18, color: 'gray', padding: w * 0.05}}>{data[1][1]}</Text>
                {data.slice(2, data.length).map(item => {
                    if(item[1])
                        return(
                            <View style={{ marginBottom: 15, alignSelf: 'center'}}>
                                <Text style={{  fontSize: 17, color: '#006AB3', marginBottom: 5}}>{params[item[0]]}</Text>
                                <View style={{ padding: 10, width: w * 0.9, backgroundColor: theme.blockColor, borderRadius: 15, elevation: 5}}>
                                    <Text style={{ fontSize: 14, color: theme.labelColor}}>{item[1]}</Text>
                                </View>
                            </View>
                        )
                })} 
