    ...
    return(
        <View style={{flex: 1, backgroundColor: theme.primaryBackground}}>
            ...
            <View>
                {
                    menu.length === 0 ? 
                    <Text style={{ fontSize: 18, alignSelf: 'center', marginTop: 20, color: theme.labelColor}}>{locale['canteens_dont_work']}</Text> : null
                }
                <FlatList 
                    data={menu}
                    renderItem={({ item }) => 
                        <TouchableOpacity 
                            ...
                            <Text style={{ fontSize: 16, color: theme.labelColor}}>{item.name}</Text>
                        </TouchableOpacity>
                    }
