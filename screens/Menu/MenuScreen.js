    ...
    return( 
        <View style={{flex: 1, backgroundColor: theme.primaryBackground}}>
            ...
            <View>
                {
                dayList.length === 0 ?
                <Text style={{ fontSize: 18, alignSelf: 'center', marginTop: 20, color: theme.labelColor}}>{locale['no_menu']}</Text> : 
                <FlatList 
                    data={dayList.menu}
                    renderItem={({ item }) => 
                        {console.log(item)
                        return(
                            <View>
                                <Text style={{ marginLeft: w * 0.05, fontSize: 18, color: '#006AB3', marginTop: 20}}>{item.type}</Text>
